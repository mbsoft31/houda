<?php

return [

    [
        'title' => "realisation d'un site web pour la gestion des projets de fin d'etudes",
        'diploma' => "master",
        'speciality_id' => 3,
        'teacher' => "abdkrim@mail.com",
        'active' => "1",
        'objective' => "jqshgdkjfhqg q ksgjd lfqkjflqKJHSDLK",
        'resume' => "dkfg czkuidqjchlizqhdmscifla zeioyo azHOD HC"
    ],
    [
        'title' => "point d'assistance et d'information",
        'diploma' => "master",
        'speciality_id' => 2,
        'teacher' => "imad@mail.com",
        'active' => "1",
        'objective' => "jqshgdkjfhqg q ksgjd lfqkjflqKJHSDLK",
        'resume' => "dkfg czkuidqjchlizqhdmscifla zeioyo azHOD HC"
    ],
    [
        'title' => "revamping of the network campus university",
        'diploma' => "master",
        'speciality_id' => 3,
        'teacher' => "jalel@mail.com",
        'active' => "1",
        'objective' => "jqshgdkjfhqg q ksgjd lfqkjflqKJHSDLK",
        'resume' => "dkfg czkuidqjchlizqhdmscifla zeioyo azHOD HC"
    ],
    [
        'title' => "Etudier la maintenabilite de logiciel a partir de son historique de l'evolution",
        'diploma' => "master",
        'speciality_id' => 6,
        'teacher' => "hanene@mail.com",
        'enterprise' => null,
        'active' => "1",
        'objective' => "jqshgdkjfhqg q ksgjd lfqkjflqKJHSDLK",
        'resume' => "dkfg czkuidqjchlizqhdmscifla zeioyo azHOD HC"
    ],
    [
        'title' => "gestion de reservation d'hotels en ligne",
        'diploma' => "master",
        'speciality_id' => 2,
        'teacher' => "youcef@mail.com",
        'active' => "1",
        'objective' => "jqshgdkjfhqg q ksgjd lfqkjflqKJHSDLK",
        'resume' => "dkfg czkuidqjchlizqhdmscifla zeioyo azHOD HC"
    ],


];
